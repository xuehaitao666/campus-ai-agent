...
def test_app_simple_non_streaming(mock_agent_client):
    ...
    WELCOME_START = "你好，我是 Campus AI Agent"
    PROMPT = "请问请假流程是什么？"
    RESPONSE = "请假流程通常包括提交申请、审批和备案。"
...
def test_app_thread_id_history(mock_agent_client):
    ...
    HISTORY = [
        ChatMessage(type="human", content="这周有什么校园活动？"),
        ChatMessage(type="ai", content="这周有技术分享会和社团招新活动。"),
    ]
    ...
    assert at.chat_message[0].avatar == "user"
    assert at.chat_message[0].markdown[0].value == "这周有什么校园活动？"
    assert at.chat_message[1].avatar == "assistant"
    assert at.chat_message[1].markdown[0].value == "这周有技术分享会和社团招新活动。"
    assert not at.exception
