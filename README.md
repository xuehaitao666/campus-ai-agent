...
Campus AI Agent 校园智能助理是一个面向大学生校园场景的 AI Agent，支持课程表查询、校园活动查询、校园制度问答和学习计划生成。

It includes a [LangGraph](https://langchain-ai.github.io/langgraph/) agent, a [FastAPI](https://fastapi.tiangolo.com/) service to serve it, a client to interact with the service, and a [Streamlit](https://streamlit.io/) app that uses the client to provide a campus assistant chat interface. Data structures and settings are built with [Pydantic](https://github.com/pydantic/pydantic).

This project offers a template for building campus-oriented agents using the LangGraph framework. It demonstrates a complete setup from agent definition to user interface, making it easier to connect campus services, knowledge bases, RAG retrieval, and student-facing assistant workflows.
...
# uv is the recommended way to install the project, but "pip install ." also works
...
1. **Campus AI Agent**: A customizable campus assistant built using the LangGraph framework for course lookup, campus activity discovery, policy Q&A, and learning plan generation.
1. **FastAPI Service**: Serves the campus agent with both streaming and non-streaming endpoints.
1. **Advanced Streaming**: A novel approach to support both token-based and message-based streaming.
1. **Streamlit Interface**: Provides a user-friendly chat interface for interacting with the campus assistant, including voice input and output.
1. **Multiple Agent Support**: Run multiple campus assistant agents in the service and call by URL path. Available agents and models are described in `/info`
1. **Asynchronous Design**: Utilizes async/await for efficient handling of concurrent requests.
1. **Content Moderation**: Implements Safeguard for content moderation (requires Groq API key).
1. **RAG Agent**: A basic RAG agent implementation using ChromaDB for campus policy or handbook retrieval - see [docs](docs/RAG_Assistant.md).
...
response = client.invoke("请问请假流程是什么？")
response.pretty_print()
# ================================== Ai Message ==================================
#
# 请假流程通常包括提交申请、辅导员或任课教师审批、学院备案等环节。
# 具体要求应以学校最新制度为准。
